...
System Admin accounts automatically have Viewer (read‐only) access to all projects and environments in the organization, regardless of explicit membership. They can edit (Editor role) only in the environments where they are assigned an Editor role or where they are the organization admin.
