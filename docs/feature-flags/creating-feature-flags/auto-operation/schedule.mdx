...
Note that the schedule option is not available in the flag switch confirmation modal on the feature flags list page. To schedule a flag change, use the Operation page and wait for the upcoming scheduling feature that will support this workflow.
