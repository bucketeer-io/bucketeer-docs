...
Progressive rollout now supports flags that have three or more variations. When configuring a rollout, you can explicitly select the **Control variation** (the variation currently served) and the **Target variation** (the variation to be rolled out) using the new `control_variation_id` and `target_variation_id` fields. The rollout gradually shifts traffic from the control variation to the target variation while any additional variations remain unchanged.
...
Progressive rollout now works for flags with three or more variations, allowing you to choose both the original (control) and the new (target) variations via the dedicated fields. This gives you full control over which variations participate in the rollout.
