...
If the flag is disabled when a progressive rollout is created, the progressive rollout automatically enables the flag as soon as it starts. The progressive rollout can only enable a flag once when it starts.
...
The progressive rollout is designed to work exclusively with flags having two variations. 
