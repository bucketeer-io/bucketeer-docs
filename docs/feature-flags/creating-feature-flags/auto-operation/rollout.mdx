...
:::caution Number of variations

The progressive rollout only works for flags with two variations. 

:::

:::caution Disabled Flag behaviour

If a progressive rollout is created for a disabled flag, the progressive rollout automatically enables the flags as soon as it starts. The progressive rollout can only enable a flag once, when it starts.

If a flag with an active progressive rollout is turned OFF, the system will automatically stop the progressive rollout. 
:::
...
It's vital to notice that executing a Kill Switch, triggered by schedule or event rate conditions, has higher priority than the progressive rollout. Therefore, if your flag has a running progressive rollout and a Kill Switch operation is triggered, the system will stop the progressive rollout. The progressive rollout will be moved to the **Finished** tab, where you can check the rollout evolution before being stopped.
...
Editing a running progressive rollout is not supported in Bucketeer. You can remove or stop the rollout.

If you remove the rollout, no historical data will be stored. Only logs will report the operations related to that rollout.

When you stop the rollout, it's shut, moving it to the finished tab, where you can inspect the rollout level it reached, for example.
