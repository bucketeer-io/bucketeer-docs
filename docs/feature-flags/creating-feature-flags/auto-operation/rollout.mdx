...
A rollout shifts traffic from one variation to another. When you configure it, you choose two variations:

- **Control variation**: the variation your users receive today.
- **Target variation**: the variation you want to roll out.

The rollout gradually moves users from the control variation to the target variation. Both variations must be different, and any other variations the flag has remain untouched during the rollout.

For example, consider that you have a boolean flag with two variations: `true` and `false`. This flag determines whether new content is available to users or not. 

Currently, the `false` variation is the [OFF variation](/feature-flags/creating-feature-flags/targeting#the-targeting-page), meaning users can't see the new content. However, you want to make the content available to all users within ten days. In this case, you can select `false` as the control variation and `true` as the target variation, and progressively roll out the `true` variation to 10% of your users daily. 
...
You can't create a progressive rollout for a flag that has a **waiting or running experiment**.

You can have only one active rollout for each feature flag.

:::

:::info Flags with three or more variations

Progressive rollout works with any number of variations. Because you select the control and the target variation explicitly, the rollout only shifts traffic between those two, and users receiving any other variation are not affected.
...
- [**Template**](/feature-flags/creating-feature-flags/auto-operation/progressive-rollout#template-rollout): With this option, you define the percentage increment and the frequency of the rollout. The weight of each phase and the execution time are automatically defined until the rollout reaches 100%.
- [**Manual**](/feature-flags/creating-feature-flags/auto-operation/progressive-rollout#manual-rollout): In this case, you are responsible for defining each rollout phase's percentage weight and release time.

### Template rollout

If you choose to use the Template rollout, you need to define the following configurations before submitting it:

- **Control variation**: Choose the variation your users receive today, which traffic is moved away from.
- **Target variation**: Choose the variation to roll out. It must be different from the control variation. You should not choose the same variation selected as [OFF variation](/feature-flags/creating-feature-flags/targeting#the-targeting-page). Otherwise, it could affect what the end users see.
- **Start Date**: The rollout starting date and time. 
- **Increment**: The percentage increment used at each rollout release phase. This option will define, as a result, the total number of rollout releases. If you choose 10%, ten stages are required to reach the total rollout.
...
- **Control variation**: Choose the variation your users receive today, which traffic is moved away from.
- **Target variation**: Choose the variation to roll out. It must be different from the control variation. You should not choose the same variation selected as [OFF variation](/feature-flags/creating-feature-flags/targeting#the-targeting-page). Otherwise, you will receive an error message.
- **Weight**: The percentage of users receiving the target variation at the selected date. 
...
You can't create a progressive rollout for a flag that has a waiting or running experiment. The rollout itself works with any number of variations, because you select the control and the target variation explicitly.

If you create a progressive rollout and the flag default strategy uses a [variation allocation percentage](/feature-flags/creating-feature-flags/targeting/default-rule#variation-allocation), the existing rollout rules will be overridden. The default strategy will be reset according to the settings defined during the progressive rollout creation.
