...
The Trigger URL includes the authentication token, so all you need to do to activate the trigger is to perform a POST request using the Trigger URL. You must keep the Trigger URL safe and secure, as anyone with access to it can modify the state of your system flag.

:::warning Trigger URL linkage

In the event of a Trigger URL leak, the Bucketeer team recommends you immediately reset or delete the trigger. Otherwise, anyone with the Trigger URL can modify the state of your application.
