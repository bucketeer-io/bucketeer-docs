...
You can change the order of custom rules directly in the UI by moving a rule up or down in the list. The updated sequence is stored via the `rule_order` field, so the new evaluation order is retained.
