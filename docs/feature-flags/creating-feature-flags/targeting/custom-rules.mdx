...
Target predefined groups of users by their IDs or by attribute-based rules. Create reusable segments like "Beta Testers", "VIP Users", or "Internal Team" and apply them across multiple flags.

**How it works:**
User segments allow you to create user groups based on the unique ID of end users, on attribute-based segment rules, or both. A user belongs to the segment if their ID is in the uploaded user list or if they match any of the segment rules. Once you've created a segment, you can use it in your targeting rules to assign variations to all users in that segment.
...
- Rule-based membership - target users by attributes without maintaining ID lists (see [Segment rules](/feature-flags/segments#segment-rules))
...
Custom rules are evaluated in the order they appear. Place more specific rules before general ones. The first matching rule wins and returns its variation. You can change the order by moving rules up or down in the UI, and the updated sequence is persisted via the `rule_order` field.
:::
