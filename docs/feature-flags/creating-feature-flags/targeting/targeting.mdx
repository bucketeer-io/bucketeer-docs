...
#### User Segments
Target predefined groups of users by their IDs. Create reusable segments like "Beta Testers", "VIP Users", or "Internal Team" and apply them across multiple flags. When a rule includes a SEGMENT clause with multiple segment IDs, the clause matches if the user belongs to any of the listed segments (OR logic). To require a user to be in all of several segments, add separate SEGMENT clauses for each segment within the same rule.
