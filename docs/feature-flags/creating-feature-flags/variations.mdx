...
At the top of the Variations tab a **Value Schema** card lets you define a schema for the variation values of the flag. Supported schema types are Enum (for string or number variations), Regex (for string variations), and JSON Schema (for JSON variations). When a schema is active, the UI validates each variation value against it and displays inline validation errors—such as disallowed enum values, pattern mismatches, or JSON schema violations—preventing the variation from being saved until the errors are resolved.
