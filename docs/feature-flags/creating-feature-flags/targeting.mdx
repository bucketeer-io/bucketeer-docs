...
When defining rollout rules based on other feature flags, consider the performance impact on your system. If a flag to be evaluated has many dependencies, evaluating one flag may also evaluate many other flags, potentially affecting the performance of evaluation duration and client's network load.
:::
