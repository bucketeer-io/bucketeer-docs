...
A user segment is a specific group of users defined within the dashboard based on their unique `id`. Each user's `id` is set when you create the user using the `defineBKTUser` function. Users can be associated with segments manually within the dashboard or by uploading **.txt** or **.csv** files. 

:::info Create segments
You need to create segments before using them in rollout rules. Access the [Segments](/feature-flags/segments) page to learn how to create and manage segments.
:::
