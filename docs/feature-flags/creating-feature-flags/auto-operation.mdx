...
1. **Operation**: An operation is an action that is triggered by the condition. The available operations are:
   ...
   - _Enable_: Activates the flag.
   - _Kill Switch_: Desables the flag.
...
After a condition is triggered, the related operation changes from active to complete. As a result, the operation is moved from the **Active** to the **Completed** panel.
