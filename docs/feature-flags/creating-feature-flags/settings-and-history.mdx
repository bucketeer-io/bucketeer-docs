...
In the Settigs page, you can also clone or archive the flag. Cloning creates a copy of the current flag with the same configurations as the original, while archiving removes the flag from all environments but retains its history for future reference.
