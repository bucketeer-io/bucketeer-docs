...
Segments can also define membership with attribute‐based rules. In the segment editor a **rule builder** lets you add one or more rules; conditions inside a rule are combined with **AND**, while multiple rules are combined with **OR**. A user belongs to the segment if they appear in the uploaded user‐ID list **or** they satisfy any of the defined rules. When a segment is referenced by a flag, any change to the segment’s rules or user list is applied automatically to all flags that use that segment.
...
You can update segments that are being used by feature flags, including the user list. However, if these flags are part of active experiments, changing the segment may affect result accuracy since it modifies which users are included in each variation.
...
After creating a User Segment, you can download the user's list. To do this, access the User Segment page, find the desired segment list and click the action menu (three dots). After, select **Download User Segment**.
