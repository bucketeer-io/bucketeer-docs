...
5. **Check events**: Each flag has an Evaluation page where you can track its usage. To check usage data, access the dashboard, go to **Feature Flags**, select the desired flag, and switch to the **Evaluation** tab. If recorded data exists on this page, your implementation was successful. Otherwise, revisit the previous steps and make sure you executed tests using the just created flag. The image below presents an example of a correct integration.
