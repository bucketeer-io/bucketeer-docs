...
To create a feature flag on the Bucketeer dashboard, access the **Feature Flags** tab. You will find all existing flags available on the current environment on this page. To add a new one, click **+ Add**. A side panel will appear on the right side of the screen. You can also launch the built-in onboarding walkthrough via the **Get Started** button at the bottom of the sidebar, which will guide you through creating a flag.
